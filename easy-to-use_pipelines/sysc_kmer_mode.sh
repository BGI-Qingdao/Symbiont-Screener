...
usage of example pipeline - sysc_strobmer_mode.sh:
    sysc_kmer_mode.sh     <--maternal mat.ngs> \\
                          <--paternal pat.ngs> \\
...
#################################################################
$SEQKIT grep -f step02.2.k21/trio_only.filtered_readname.txt \
                $SONTGS >host.fa

$SEQKIT grep -v -f step02.2.k21/trio_only.filtered_readname.txt \
                $SONTGS > symbiont.fa
